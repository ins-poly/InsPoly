...
- For bounded indexer follow-up, preserve the target registry, scoped compare tooling, and new public-trade collection metadata. Before warehouse RFC work, harden public-trade collection to avoid active-target sampling displacing anchor trades, preferably through per-target/per-condition caps and cursor identity. Preserve narrow caps, fresh local-only DB output, readiness audit, and no warehouse/runtime/report integration.
