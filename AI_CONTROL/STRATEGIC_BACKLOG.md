...
- If indexer work continues, use the bounded live-indexer approval packet, config validator, manual sidecar runner, first-run report, repeat-run hardening RFC, DB compare tool, same-slug repeat probe report, and multi-target partial-success report first. Before another multi-target live run, harden target selection/provider lookup and add scoped sidecar DB comparison; do not start repeat ingestion, background workers, warehouse mode, target expansion beyond approved caps, report/browser integration, storage schema changes, or production integration without separate approval.
...
- For bounded indexer follow-up, draft target-hardening and scoped-compare work before approving a repeat multi-target run. Preserve narrow caps, fresh local-only DB output, readiness audit, and no warehouse/runtime/report integration.
