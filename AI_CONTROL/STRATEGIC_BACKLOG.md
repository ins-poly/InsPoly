...
- RC V4 is now published as draft PR `#1`: https://github.com/ins-poly/InsPoly/pull/1. Current PR-review gate: `rc_v4_pr_stay_draft_needs_ci_or_review`. Do not start another local feature campaign unless explicitly separated from RC V4 publication.
- Use Release Candidate V4 as the current published draft baseline. Before marking ready or merging, review `docs/inspoly_rc_v4_pr_review_status_20260528.md`, `docs/inspoly_rc_v4_pr_reviewer_risk_audit_20260528.md`, `docs/inspoly_rc_v4_pr_local_verification_20260528.md`, and `docs/inspoly_rc_v4_pr_merge_readiness_decision_20260528.md`.
- Next owner decisions for PR #1: whether to accept absent GitHub CI or configure checks, whether to manually update the PR body with reviewer doc links, whether historical `release_manifests/` files should stay in the PR, whether the committed local-path redaction patch satisfies privacy/publication hygiene, and whether to approve marking the Draft PR ready for review. Merge remains a separate explicit approval step.
...
## Should Do
- If the owner asks to mark PR #1 ready, first confirm review/CI expectations and packaging-artifact decision. If the owner asks to merge, first confirm ready/review state and produce a merge-readiness checkpoint. Do not merge from a general maintenance campaign.
