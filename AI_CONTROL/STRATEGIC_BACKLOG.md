...
- If indexer work continues, use the bounded live-indexer approval packet, config validator, manual sidecar runner, first-run report, repeat-run hardening RFC, and DB compare tool first. A second same-slug fresh-DB probe is ready for explicit operator approval, but do not start repeat ingestion, background workers, warehouse mode, target expansion, report/browser integration, storage schema changes, or production integration without that separate approval.
...
- For bounded indexer follow-up, if the owner approves one second same-slug probe, run it into a fresh local-only sidecar DB and compare it with `tools/indexer_sidecar_db_compare.py --comparison-mode fresh_live`; then audit drift before any warehouse/runtime RFC.
