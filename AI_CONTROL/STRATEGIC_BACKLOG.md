...
- For bounded indexer follow-up, preserve the target registry, scoped compare tooling, per-target public-trade collection metadata, warehouse RFC packet, W0 contract helper/readiness audit fields, W1 manual local review command, and W2 registry/retention manifest. Next highest-leverage work is W3 analyst sidecar query/read: answer analyst questions from compact registry and W1 summaries only, with no copied report metrics, no report/browser integration, no production imports, no live ingestion, no warehouse writer/copy behavior, and no scheduler/background mode unless separately approved.
