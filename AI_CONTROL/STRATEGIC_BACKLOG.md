...
- RC V4 is now published as PR `#1`: https://github.com/ins-poly/InsPoly/pull/1. Current PR-review gate: `rc_v4_ready_to_merge_owner_approval_required`. Do not start another local feature campaign unless explicitly separated from RC V4 publication.
...
- Next owner decision for PR #1: merge now based on the final local pre-merge checkpoint, or keep it open and require GitHub CI/human review first. Tracked historical `release_manifests/` files have been cleaned from the PR branch; merge remains a separate explicit approval step.
...
## Should Do
- If the owner asks to merge PR #1, confirm they accept the no-CI/no-review GitHub posture and then merge only under explicit merge instruction. Do not merge from a general maintenance campaign.
