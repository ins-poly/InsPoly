...
- For bounded indexer follow-up, preserve the target registry, scoped compare tooling, per-target public-trade collection metadata, and warehouse RFC packet. Next highest-leverage work is W0 no-runtime schema/readiness cleanup: old sidecar DB compatibility, per-condition cursor design choice, retention/deletion policy, readiness/compare contract refinement, and implementation guardrails. Preserve no warehouse write command, no live ingestion, and no warehouse/runtime/report integration unless separately approved.
