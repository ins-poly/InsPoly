...
- If indexer work continues, use the bounded live-indexer approval packet, config validator, manual sidecar runner, first-run report, repeat-run hardening RFC, DB compare tool, same-slug repeat probe report, multi-target partial-success report, target registry, resolver preflight, scoped compare output, and multi-target hardening decision first. Current gate: `indexer_multitarget_hardening_ready_for_repeat_operator_run`. The next safe indexer step is a separately approved repeat over the same three-target set, or per-target public-trade collection hardening before any warehouse RFC. Do not start repeat ingestion, background workers, warehouse mode, broader target expansion, report/browser integration, storage schema changes, or production integration without separate approval.
...
- For bounded indexer follow-up, preserve the target registry and scoped compare tooling. Before warehouse RFC work, either repeat the same three-target set under operator approval or harden public-trade collection to avoid active-target sampling displacing anchor trades. Preserve narrow caps, fresh local-only DB output, readiness audit, and no warehouse/runtime/report integration.
