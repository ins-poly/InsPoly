...
- If indexer work continues, use the bounded live-indexer approval packet, config validator, manual sidecar runner, first-run report, repeat-run hardening RFC, DB compare tool, and same-slug repeat probe report first. A small multi-target operator run is ready for a separate approval packet, but do not start repeat ingestion, background workers, warehouse mode, target expansion beyond approved caps, report/browser integration, storage schema changes, or production integration without that separate approval.
...
- For bounded indexer follow-up, draft a small multi-target operator approval packet using the clean same-slug repeat evidence. Keep caps narrow, write to a fresh local-only DB, run readiness plus `tools/indexer_sidecar_db_compare.py`, and stop before any warehouse/runtime RFC.
