...
- RC V4 is now published as draft PR `#1`: https://github.com/ins-poly/InsPoly/pull/1. Current PR-review gate: `rc_v4_ready_for_owner_mark_ready_review`. Do not start another local feature campaign unless explicitly separated from RC V4 publication.
...
- Next owner decisions for PR #1: whether to accept absent GitHub CI or configure checks, whether to manually update the PR body from `docs/inspoly_rc_v4_pr_body_replacement_20260528.md`, and whether to mark the Draft PR ready for review. Tracked historical `release_manifests/` files have been cleaned from the PR branch; merge remains a separate explicit approval step.
...
## Should Do
- If the owner asks to mark PR #1 ready, first confirm no-CI acceptance and whether the PR body has been updated manually. If the owner asks to merge, first confirm ready/review state and produce a merge-readiness checkpoint. Do not merge from a general maintenance campaign.
