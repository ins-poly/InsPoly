...
- For bounded indexer follow-up, preserve the target registry, scoped compare tooling, per-target public-trade collection metadata, warehouse RFC packet, W0 contract helper/readiness audit fields, and W1 manual local review command. Next highest-leverage work is W2 registry/retention: target registry format, local DB inventory, retained-run manifest, deletion/byte limits, stale DB policy, and rollback. Preserve no live ingestion, no warehouse writer/copy behavior, no scheduler/background mode, and no warehouse/runtime/report integration unless separately approved.
