...
- If indexer work continues, use the bounded live-indexer approval packet, config validator, manual sidecar runner, first-run report, repeat-run hardening RFC, DB compare tool, same-slug repeat probe report, multi-target reports, target registry, resolver preflight, scoped compare outputs, repeatability/collection campaign, per-target collection hardening campaign, warehouse RFC packet, W0 contract/readiness packet, W1 manual-command packet, W2 registry/retention packet, W3 analyst query packet, and W4 report-pointer decision packet first. Current gate: `indexer_w4_pointer_metadata_rfc_ready`. The next safe indexer step is a separately approved W4 pointer-metadata implementation campaign, if the owner wants report-level discoverability; keep it pointer-only with no copied metrics, no row-level sidecar context, no UI panel by default, no scoring/gate effect, no production sidecar imports, no live ingestion, no warehouse writer/copy behavior, no storage migration, and no scheduler/background mode unless separately approved.
...
- For bounded indexer follow-up, preserve the target registry, scoped compare tooling, per-target public-trade collection metadata, warehouse RFC packet, W0 contract helper/readiness audit fields, W1 manual local review command, W2 registry/retention manifest, W3 analyst query command, and W4 pointer decision packet. Next highest-leverage work is W4 pointer metadata implementation only after explicit product approval; keep it one optional top-level pointer with no copied metrics, no UI panel by default, no scoring/gate effect, no production imports, no live ingestion, no warehouse writer/copy behavior, and no scheduler/background mode unless separately approved.
