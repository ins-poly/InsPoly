...
- For bounded indexer follow-up, preserve the target registry, scoped compare tooling, and per-target public-trade collection metadata. Next highest-leverage work is an RFC-only sidecar warehouse plan covering target registry rules, per-condition cursor options, retention/deletion policy, operator caps, readiness audit gates, scoped compare gates, and rollback. Preserve narrow caps, fresh local-only DB output, and no warehouse/runtime/report integration.
