...
- Live indexer/warehouse work now has an operator approval packet, no-network config validator, manual sidecar runner, and one successful bounded probe. It still needs operator hardening/repeat-run RFC before any scheduling, warehouse mode, production integration, report/UI wiring, or broader target expansion.
